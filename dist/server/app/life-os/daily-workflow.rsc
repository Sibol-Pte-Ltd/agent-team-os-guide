...
5:T1514,<div class="breadcrumbs">
        ...
        <p>If something comes up during the day that you want to capture, drop it into your Logseq journal page. A quick bullet point is enough. The agents monitor Notes for new entries and will incorporate anything you add into their ongoing work. You can also message Ember directly if you need something handled urgently or want to reprioritize.</p>

        <div class="callout note">
          <div class="callout-title">When Do Agents See My Changes?</div>
          <p>Each time an agent runs — whether triggered by a cron job, a direct message, or a heartbeat — the Agent Runner assembles workspace context from the current state of Notes files on disk. This means any changes you make to Notes are immediately visible to agents on their next run. There is no cache to invalidate or refresh button to press. As long as <a href="/troubleshooting/syncthing">Syncthing</a> has delivered your changes to the EC2 instance, agents will see them.</p>
        </div>

        <h3>Evening</h3>

        <p>At 9:30 PM, the <a href="/cadence/evening-checkin">evening check-in</a> arrives via your notification channel. Like the morning report, this is an OpenClaw cron job — the Agent Runner spins up a fresh execution context, loads the current state of Notes, and Ember generates a brief summary of what was accomplished during the day, both by you and by the agents. It also prompts you to reflect on the day: what went well, what did not, and what should carry forward to tomorrow.</p>
...
a:null
c:{"metadata":[["$","title","0",{"children":"Daily Workflow — Agent Team OS Guide"}],["$","meta","1",{"name":"description","content":"Your personal AI agent team, working together to keep your life organized."}]],"error":null,"digest":"$undefined"}
11:"$c:metadata"
